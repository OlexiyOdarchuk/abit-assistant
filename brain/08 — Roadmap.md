...
**Лишилось для №2:** (1) скрейпер код-таблиць регіон/спеціальність із
`<select>` форми (для UX вибору); (2) сервіс reverse-search:
BrowsePrograms → Fetch кожну → Analyze → reach/match/safety; (3) bot UX.
**Effort:** ~1.5 дня.

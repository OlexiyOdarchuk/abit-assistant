...
**Невідоме (→ deep research / reverse-eng):** osvita не має чистого API
переліку програм за регіон+спеціальність. Треба знайти browse-endpoint
(сторінки `vstup.osvita.ua` за областю/галуззю віддають список program URL)
або взяти перелік з abit-poisk/EDBO. Це блокер фічі.

**Effort:** enumeration-джерело (1–2 дні reverse-eng) + bulk-Analyze поверх
наявного (1 день). Висока цінність: відповідає на головне питання абітурієнта.

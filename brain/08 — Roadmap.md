...
**Endpoint ЗНАЙДЕНО (deep research 2026-06-13):** `vstup.osvita.ua/spec/`
браузить програми за рівнем/регіоном/галуззю/спеціальністю/формою через
числові слоти URL. Деталі й приклади → [[05 — Парсери і сервіси#Browse-endpoint `/spec/` — розблоковує реверс-пошук (фіча №2)|05 — browse-endpoint]].
Лишилось зняти повну мапу слотів з живого `/spec/0-0-0/` (open question).

**Effort:** дописати `/spec/` enumeration-parser + bulk-Analyze поверх
наявного (~2 дні). Висока цінність: відповідає на головне питання абітурієнта.

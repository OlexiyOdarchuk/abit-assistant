...
Логіка `IsCompetitor` у `internal/abit/filter.go`. Marker рендериться у `applicantButtonLabel()`. Override на детальному екрані — `handleToggleThreat()` мутує `state.Data["overrides"]` мапу. Поки `search.viewing` стейт активний з тим самим URL — overrides sticky.
